...
if [ $# -lt 3 ]; then
    ...
    echo "  ./inference_bbfm.sh model in.s16 out.wav [optional inference_bbfm.py args]"
    echo "usage (play output with aplay):"
    echo "  ./inference_bbfm.sh model in.s16 - [optional inference_bbfm.py args]"
    exit 1
fi
...
python3 ./inference_bbfm.py ${model} ${features_in} ${features_out} "$@"
if [ $? -ne 0 ]; then
  exit 1
fi

...
1. Multipath demo at approx 0dB B=3000 Hz SNR. First generate multipath channel samples using GNU Octave (only need to be generated once): 
   ...
   $ ./inference.sh model03/checkpoints/checkpoint_epoch_100.pth ~/LPCNet/wav/all.wav tmp.wav --EbNodB 3 --write_latent z_hat.f32 --mp_file h_mpp.f32
   ```
...
1. Baseline no noise simulation:
   ```
   cp ~/codec2-dev/build_linux/unittest/fast_fading_samples.float g_mpp.f32.
   ...
   ```
   Note g_mpp.f32 uses Doppler sperads files generated by codec2 build process for ch tool (TODO Octave script details)

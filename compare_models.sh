...
# run a single training epoch on each trained model to build loss versus Eq/No curve
function run_model() {
    model=$1
    dim=$2
    shift
...
#run_model model12 40  --q_opt  # tanh applied to |z|, --range_EbNo_start 0

echo "radae_plots; loss_EqNo_plot('loss_EqNo_models','model05_loss_EqNodB.txt','m5 dim 80 mpp #1','model09_loss_EqNodB.txt', \
     'm9 dim 80 mpp #2','model10_loss_EqNodB.txt','m10 dim 40 awgn 1D','model11_loss_EqNodB.txt','m11 dim 40 awgn 2D #1', \
...
echo "radae_plots; loss_CNo_plot('loss_CNo_models', 50, 1, \
                                 ...
                                 'model09_loss_EqNodB.txt',80,'m9 dim 80 mpp 1D #2', \
                                 'model10_loss_EqNodB.txt',40,'m10 dim 40 awgn 1D', \
                                 'model11_loss_EqNodB.txt',40,'m11 dim 40 awgn 2D #1', \

...
The April 2024 stored file OTA test campaign showed the need for a method of measuring the SNR from off air RADAE signals. Consider the normalised cross correlation function from \cite{freedv_low}:
\begin{equation}
\label{eq:norm_cross}
...
\end{split}
\end{equation}
This algorithm was simulated in \emph{est\_snr.py} with reasonable results over the range -10 to +20 dB SNR (Figure \ref{fig:est_snr}). While there is significant variance in the estimates at low SNRs, the mean appears correct therefore averaging SNR estimates over multiple modem frames should lead to reasonable SNR estimates.

\begin{figure}[H]
\caption{Scatter plot of actual versus estimated SNR values for a 15 carrier waveform, $M=160$ sample pilot sequence.}
\label{fig:est_snr}
\begin{center}
\input est_snr.tex
\end{center}
...
Further work:
\begin{itemize}
\item What is the noise bandwidth, $F_s$?  What happens when we BPF?
...
\item It may be better to choose a time shifted version of $\bm{p}$ as the prototype to avoid the sequence including ISI which would bias the SNR results.
\end{itemize}
\section{Glossary}

...
Now consider the pilot sequence passing through a two path multipath channel simulation, where \emph{path1} is the fastest path to our receiver and \emph{path2} the slowest. The timing estimates $t$ that maximises $D_t$ will tend to point to the last $M$ samples of either \emph{path1} or \emph{path2}, depending on which path best maximnises $D_t$.  The two paths are time varying, but can be considered stationary over the timing estimator analysis window.  We denote the two possible timing estimates $t_1$ and $t_2$. Given $t_1$ or $t_2$, we wish to determine a $M$ sample window of samples starting at $t_{sam}$ for sampling the received OFDM symbol that minimise Inter symbol Interference (ISI).

We set a timing reference at $t=0$ samples, the start of the received symbol for \emph{path1}.

\begin{figure} [H]
\caption{Multipath channel, sum of signals from two paths.}
\vspace{5mm}
\label{fig:dt_max}
\begin{center}
\begin{tikzpicture}[>=triangle 45,x=1.0cm,y=1.0cm]
...
\draw (1.5,3.5) -- (5.5,3.5) -- (5.5,4.5) -- (1.5,4.5) -- (1.5,3.5);
\draw (2.5,4.5) -- (2.5,4.5);
\node[align=left] at (0.5,4) {path2};

\draw [very thin,gray,->] (1,2.25) -- (1,3);
\node[align=center] at (1,2) {$t=0$};

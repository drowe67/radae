...
\end{equation}
and $\bm{r}$ is a vector of off air received pilot samples.  By insertion of the Cyclic Prefix we extend $r(n)$ to $N_{cp}+M$ samples, where any $M$ sample subset of $r_t(n)$:
\begin{equation}
r_t(n) = gp(n+t) \quad n=0,..,M, \quad -(N_{cp}-1) \le t \le 0
\end{equation}
is periodic in $M$ and $g$ is a complex scalar gain. The correlation as a function of the time offset $t$ is given by:
\begin{equation}
\label{eq:dt_time_offset}
\begin{split}
D_t &= \sum_{n=0}^{M-1} \sum_{c=1}^{Nc}r(n+t)p(n) \\
    &= g \sum_{n=0}^{M-1} \sum_{c=1}^{Nc} |P_c|^2 e^{-j \omega_c t}
\end{split}
\end{equation}
A peak in $|D_t|$ may occur for any $-(N_{cp}-1) \le t \le 0$, however by inspecting (\ref{eq:dt_time_offset}) we can see that for $t \ne 0$ the phase shift term results in a reduction in $|D_t|$.  Therefore the maxima of $|D_t|$ will tend to be located at the $t=0$ point (Figure \ref{fig:dt_max}).
...
\node[align=center] at (1.5,5.5) {CP};
\node[align=center] at (3.5,5.5) {Pilot Samples};
\draw [<->] (1,6.5) -- (2,6.5); \draw [<->] (2,6.5) -- (5,6.5);
...
\draw (2,3.5) -- (5,3.5) -- (5,4.5) -- (2,4.5) -- (2,3.5);
\node[align=left] at (0.5,4) {$p(n)$};
\node[align=center] at (3.5,4) {Pilot Samples};

\draw [very thin,gray,->] (2,2.25) -- (2,3);
\node[align=center] at (2,2) {$t=0$};
...
\begin{figure} [H]
\caption{Multipath channel, our received symbol is the sum of \emph{path1} and \emph{path2}. Grey regions are ISI from past and future symbols.  From the sum we must select $M$ samples that avoids any ISI.}
\vspace{5mm}
...
\end{split}
\end{equation}
This timing algoirthm was verified by simulation using data symbols for the MPP (2ms delay spread) channel.  A similar algorithm is used for the FreeDV OFDM modes. It has the disadvantage of requiring $N_{cp}>2N_{ds}$ which is wasteful of power. Further work:
\begin{itemize}
\item This algorithm is likely to be non-optimal and researching timing estimation is a useful area for further work.
\item Additionally, in the April 2024 stored file OTA test campaign, no Cyclic Prefix was used (due to a configuration error) however good results were obtained from decoded speech samples, suggesting the RADAE decoder is quite robust to ISI, or perhaps ISI is interpreted as another source of noise and is handled by the decoder without breaking down (but perhaps with some loss in performance).
\item An objective measure to determine distortion as a function of cyclic prefix length.
\item A method of measuring and observing channel impulse responses (e.g. on international DX paths) would be very useful.
\end{itemize} 

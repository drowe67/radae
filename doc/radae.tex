...
r(n) =\begin{cases}
	...
	\end{cases}
\end{equation}
In the cases (\ref{eq:rv_cases}) we model $|D_t|$ as a Rayleigh distributed random variable with scale parameter $\sigma_r$. The cases are further explained:
\begin{enumerate}
...
\end{split}
\end{equation}
Observing that the Rayleigh scale parameter is proportional to $\sigma$, we apply this approximation to the Rayleigh scale parameter:
\begin{equation}
\label{eq:rayleigh_sum}
\sigma_{r12} \approx \sqrt{2}\sigma_r
\end{equation}
Figure \ref{fig:rayleigh_sum} shows the results of using this approximation. In the region of interest around $P_{12}(x)=\num{1e-4}$ the probability density derived from (\ref{eq:rayleigh_sum}) is around 5 times the measured histogram.  For a single Rayleigh random variable the probability of a non-pilot sample of $|D_t|$ exceeding $x$ is area of the region to the right of $x$ or $1-CDF(x)$:
\begin{equation}
P(|D_t|>x,\sigma_r) = e^{-x^2/(2\sigma_r^2)}
\end{equation}
Applying (\ref{eq:rayleigh_sum}) and the correction factor of 5 from Figure (\ref{fig:rayleigh_sum}) to the sum of two Rayleigh RVs:
\begin{equation}
\begin{split}
P_{12}(|D_{t12}|>x,\sigma_r) &\approx 5e^{-x^2/(2\sigma_{r12}^2)} \\
P_{12}(|D_{t12}|>x,\sigma_r) &= 5e^{-x^2/(4\sigma_r^2)}
\end{split}
\end{equation}

\begin{figure}[H]
\caption{Theoretical Rayleigh PDF using scale parameter from (\ref{eq:rayleigh_sum}) and simulation histogram for sum of two Rayleigh RVs $X1$ and $X2$.}
...
Consider the peak when we sum two adjacent modem frames:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
From (\ref{eq:corr_prob}) we can compare the probability of false detection of a pilot as:
\begin{equation}
\begin{split}
P &= e^{-(|D_{tmax}|/\sigma)^2} \\
P_{sum} &= e^{-(|D_{tsummax}|/\sigma_{sum})^2} \\
P_{sum} &= e^{-(|2D_{tmax}|/(\sqrt{2}\sigma))^2} \\
P_{sum} &= e^{-2(|D_{tmax}|/\sigma)^2} \\
P_{sum} &= P^2
\end{split}
\end{equation}
For example if $P=\num{1e-3}$, $P_{sum}=\num{1e-6}$ - a significant improvement. It can also be shown that the ratio $D_{tsummax}/\sigma = \sqrt(2)D_{tmax}/\sigma$, an improvement of 3dB. This is not quite the same as increasing the received signal power by 3dB, as when the signal power increases both the pilot and ``interfering" ML symbols in case 3 of (\ref{eq:rv_cases}) increases.



\subsection{Sync State Machine}

Figure \ref{fig:sync_state_mach} illustrates the basic operation of the sync state machine. In the \emph{search} state we maximise $D_t$ over a grid of time and frequency values.  When it $|D_t|>D_{thresh}$ we enter the \emph{sync} state.  In the sync state $D_t$ is sampled over a smaller range of timing offsets to track sample clock offsets and the optimum sampling instant in a multipath channel.  When the signal disappears, $D_t$ will drop beneath $D_{thresh}$ and we will return to the \emph{search} state.  The pilot detector output $d$ is defined as:
\begin{equation}
d =	\begin{cases}
      0, & |D_t| < D_{thresh} \\
      1, & |D_t| \ge D_{thresh}

...
We wish to use classical DSP techniques for acquisition (e.g. coarse timing and frequency offset estimation).  Given the lack of structure in the ML symbols, the only ``known'' part of the signal is the periodic pilot sequence $r(n)=gp(n)$, send once per modem frame.  This is typically detected by maximising $D_t$ (\ref{eq:dt}) over a grid of time and frequency offsets.  From (\ref{eq:dt_time_offset}), if we assume constant pilot magnitude $|P_c|=P$ and ideal timing the maximum is:
\begin{equation}
|D_t| = g M N_c P^2
\end{equation}
...
\begin{tabular}{l r r r r r}
 ...
 \hline
\end{tabular}
\caption{\emph{dt\_test.sh} simulation results for AWGN channel for \emph{model17} and \emph{model18} waveforms. $N_{frames}$ is the number of frames it takes to acquire using the \emph{rx.py} state machine.}
\label{tab:acq_dtmax}
\end{table}

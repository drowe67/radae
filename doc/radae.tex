...
Figure \ref{fig:bbfm_framework} illustrates signal processing steps for the prototype system. The RRC filters implicitly convert between the symbol rate $R_s$ and sample rate $F_s$ using interpolation and decimation as required. Not shown is the DAC/ADC and RF signal processing which is assumed to have minimal impact on system performance.  FSYNC is a 24 symbol sequence used for frame synchronisation and fine timing.  All of the blocks not marked \emph{radae} are implemented with classical signal processing, common to existing digital data over baseband FM radios, and requiring modest amounts of CPU and memory compared to the ML.
 
\begin{figure}[H]
\caption{Baseband FM system.  Transmitter (Tx) is on the LHS, at RHS is the Receiver (Rx).}
...
\node [block, right of=fm_mod,node distance=3cm] (channel) {Radio Channel};
\node [block, right of=channel,node distance=3cm, text width=2cm] (fm_demod) {FM Demod};
...
\draw [->] (fm_mod) -- node[below] {$F_s$} (channel); 
\draw [->] (channel) --  node[below] {$F_s$} (fm_demod);

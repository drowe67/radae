...
Figure \ref{fig:bbfm_framework} illustrates signal processing steps for the prototype system. The RRC filters implicitly convert between the symbol rate $R_s$ and sample rate $F_s$ Hz. Not shown is the DAC/ADC and RF signal processing.  FSYNC is a 24 symbol sequence used for frame synchronisation and fine timing.  All of the blocks not marked \emph{radae} are implemented with classical signal processing, common to existing digital data over baseband FM radios, and requiring modest amounts of CPU.

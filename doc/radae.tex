...
\begin{figure}[h]
\caption{Inference time PAPR optimised system, showing integration with classical DSP OFDM components. Pilot symbols $\bf{p}$ are inserted into the modem frame with a hand tuned gain $p_g$ to adjust PAPR. The pilots are constant and known to both the Tx and Rx. A Cyclic Prefix (CP) appended to each time domain symbol provides protection from ISI. Acquisition and equalisation is performed using classical pilot-based DSP.}
...
The goal is to estimate SNR over a multipath channel over a T second window.  Note this is not the expected value of the SNR, as $T$ may be small compared to the fading period. At the receiver, the power of each OFDM carrier is time varying, but noise power (and spectral density) constant.  We would like to use the pilots to estimate SNR for the time window. At the receiver, let the signal power $S$ over the time window be given by:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
where $P_c^n$ is the received freq domain pilot of carrier $c$ at time step $n$, $N_w=T/T_f$ is the number of pilot vectors in the time window, $P_c$ is the real valued transmitted pilot for carrier $c$, and $h_c^n$ is complex fading channel coefficient for carrier $c$ at time $n$.  Given $|P_c|=g$ it can be shown that:
\begin{equation}
\begin{split}
...
In practice, at the receiver we have the received pilots symbols with additive noise:
\begin{equation}
...
\begin{equation}
\label{eq:snr2_signal_noise}
\begin{split}
S_1 &= \sum_{n=0}^{N_w-1} \sum_{c=1}^{N_c} |\hat{P}_c^n|^2 \\
S_1 &= \sum_{n=0}^{N_w-1} \sum_{c=1}^{N_c} \left(|h_c^nP_c|^2 + 2Re\{h_c^nP_c\mathcal{N}(0,\sigma^2)\} + |\mathcal{N}(0,\sigma^2)|^2\right) \\
    ...
    &= \sum_{n=0}^{N_w-1} \sum_{c=1}^{N_c} |h_c^nP_c|^2 + N_w N_c |\mathcal{N}(0,\sigma^2)|^2 \\
    &= S + N
\end{split}
\end{equation} 
The approximation is due to the second term having an expected value of zero and therefore being small compared to the first and third term.

At the receiver we use least squares phase estimation and correction to obtain the phase corrected pilots:
\begin{equation}
\hat{R}_c^n = |h_c^n|P_c e^{j\mathcal{N}(0,\alpha)} + \mathcal{N}(0,\sigma^2)
\end{equation}
where $\mathcal{N}(0,\alpha)$ is a small, random, zero mean phase error. We note that given the pilots $P_c$ are real, a scatter diagram plot of $\hat{R}_c^n$ resembles BPSK symbols on the real axis with additive complex noise. The magnitude of the fading $|h_c^n|$ will cause the symbols to move along the real axis. The imaginary component is not affected by fading, only the channel noise. Consider the power of the imaginary component, and applying a similar approximation to above: 
\begin{equation}
\label{eq:snr2_noise}
\begin{split}
S_2 &= \sum_{n=0}^{N_w-1} \sum_{c=1}^{N_c} |Im\{\hat{R}_c^n\}|^2 \\
    &= \sum_{n=0}^{N_w-1} \sum_{c=1}^{N_c} |Im\{|h_c^n|P_ce^{j\mathcal{N}(0,\alpha)}\} + Im\{\mathcal{N}(0,\sigma^2)\}|^2 \\
    &\approx \sum_{n=0}^{N_w-1} \sum_{c=1}^{N_c} |Im\{\mathcal{N}(0,\sigma^2)\}|^2 \\
    &= N/2
\end{split}
\end{equation}
Using (\ref{eq:snr2_signal_noise}) and (\ref{eq:snr2_noise}) we can obtain an approximation of the (linear) signal to noise ratio as:
\begin{equation}
\frac{S}{N} = \frac{S_1}{2S_2} - 1
\end{equation}

\begin{figure}[H]
\caption{Scatter plot of actual versus estimated $Es/N_0$ values, RADE V1 waveform, AWGN channel. Solid blue line is linear regression straight line fit, black ideal.}
\label{fig:est_snr2a}
\begin{center}
\input est_snr2a.tex
\end{center}
...
This algorithm was tested in the script \emph{est\_snr.py}, which supports single point testing and generating curves over evolving multiplath channels. There was some error in the $S/N$ estimates due to (\ref{eq:snr2_noise}) being sensitive to the noise in the phase estimator $\mathcal{N}(0,\alpha)$.  The algorithm works reasonably well for AWGN, MPG and MPP channels, however the error increases with faster fading.  A straight line was fitted to the estimator output for the three channels and the average used to provide a correction to this error.

Figure \ref{fig:est_snr2a} shows the results for an AWGN channel, and Figure \ref{fig:est_snr2b} shows a straight line fit for the estimator over the three channels tested (in both figures the correction has been applied). The results were calculated over 60 second runs using the \emph{est\_snr\_curves.sh} script. As the algorithm works at the symbol rate $S/N=E_s/N_0$.  To obtain the SNR in a different noise bandwidth (e.g 3000Hz) a constant offset will need to be applied. Between -5 and 15dB, the estimator is within +/-1dB for all channels except MPP.
...
\begin{center}
\begin{tikzpicture}[>=triangle 45,node distance=2cm]

\node[state, state] (search) {search};
\node[state, state, right of=search] (sync) {sync};
\draw (search) edge[loop above] node{$d=0$} (search)
(sync) edge[loop above] node{$d=1$} (sync);
\draw [->] (search) edge[bend left, above] node{$d=1$} (sync)
(sync) edge[bend left, below] node{$d=0$} (search);
...
Figure \ref{fig:bbfm_framework} illustrates the baseband FM model. The RRC filters implicitly convert between the symbol rate $R_s$ and sample rate $F_s$ using interpolation and decimation as required. Not shown is the DAC/ADC, pre and de-emphasis, RF frequency translation, and power amplifier which in total are assumed to have a unity transfer function and minimal impact on system performance.  FSYNC is a 24 symbol sequence used for frame synchronisation and fine timing.  All of the blocks not marked \emph{radae} are implemented with classical analog or digital signal processing, common to existing analog or digital FM radios, and requiring modest amounts of CPU and memory compared to the ML.
 
\begin{figure}[H]
\caption{Baseband FM model.  Transmitter (Tx) is on the LHS, at RHS is the Receiver (Rx). The RF channel is subject to fading $|H|$ and AWGN with noise power $\sigma_c^2$.}
\label{fig:bbfm_framework}
\begin{center}
\begin{tikzpicture}[auto, node distance=2cm,>=triangle 45,x=1.0cm,y=1.0cm,align=center,text width=1.5cm]

\node [input] (rinput) (z) {};
\node [block, below of=z,node distance=1cm] (radae_enc) {\emph{radae} Encoder};
\node [block, below of=radae_enc,node distance=2cm] (insert_fsync) {Insert FSYNC};
\node [block, below of=insert_fsync,node distance=2cm] (rrc_tx) {RRC Tx Filter};
\node [block, below of=rrc_tx,node distance=2cm] (fm_mod) {FM Mod};
\node [circ, right of=fm_mod,node distance=2cm, text width=0.5cm] (mult) {$\times$};
\node [circ, right of=mult,node distance=1.5cm, text width=0.5cm] (add) {$+$};
\node [block, right of=add,node distance=2.5cm, text width=2cm] (fm_demod) {FM Demod};
\node [block, above of=fm_demod,node distance=2cm, text width=2cm] (acq) {Acquisition \& Timing};
\node [block, above of=acq,node distance=2cm,text width=2cm] (rrc_rx) {RRC Rx Filter};
\node [block, above of=rrc_rx,node distance=2cm] (remove_fsync) {Remove FSYNC};
\node [block, above of=remove_fsync,node distance=2cm] (radae_dec) {\emph{radae} Decoder};
\node [output, right of=radae_dec,node distance=2cm] (routput) {};

\draw [->] node[above,text width=1cm] {$\bf{f}$} (rinput) -- (radae_enc);
\draw [->] (radae_enc) -- node[left, text width=0.5cm] {$\bf{z}$} (insert_fsync);
\draw [->] (insert_fsync) -- (rrc_tx); 
\draw [->] (rrc_tx)-- node[left, text width=0.5cm] {$F_s$} (fm_mod); 
\draw [->] (fm_mod) -- (mult); 
\draw [->] (mult) -- (add);
\draw [->] (add) -- (fm_demod);
\draw [->] (fm_demod)-- node[left, text width=0.5cm] {$F_s$} (acq); 
\draw [->] (acq)-- node[left, text width=0.5cm] {$F_s$} (rrc_rx); 
\draw [->] (rrc_rx)-- (remove_fsync); 
\draw [->] (remove_fsync) -- node[left, text width=0.5cm] {$\hat{\bf{z}}$} (radae_dec); 
\draw [->] (radae_dec) -- node[above, align=right, text width=1cm] {$\hat{\bf{f}}$} (routput) ;

\node [input, above of=mult,node distance=1cm] (fading) {};
\node [input, above of=add,node distance=1cm] (awgn) {};
\draw [->] (fading) node[above, text width=0.75cm] {$|H|$} -- (mult); 
\draw [->] (awgn) node[above, text width=0.75cm] {$\mathcal{N}(0,\sigma_c^2)$} -- (add); 

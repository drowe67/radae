...
\end{equation}
where $\beta=f_d / f_m$ is the modulation index, CNR is the Carrier to Noise ratio of the received signal at the demodulator input, and $SNR_{FM}$ is the SNR at the demodulator output, with noise power measured in bandwidth $f_m$.  The RF bandwidth can be approximated by:
\begin{equation}
BW_{FM} = 2(f_d+f_m)
\end{equation}
...
We can model the received symbols at the output of the FM demodulator as the input symbols with additive Gaussian noise:
\begin{equation}
\label{eq:fm_noise}
\hat{\bf{f}} = \bf{f} + \mathcal{N}(0,\sigma_s^2) 
\end{equation}
where the symbol noise power $\sigma_s^2$ can be computed from the FM carrier noise power $\sigma_c^2$ using [\ref{eq:fm_snr}].  We note the mean magnitude of each symbols in $\hat{\bf{f}}$ at the output of a FM demodulator is a function of the deviation and not the channel CNR.  Thus as the CNR is modulated by fading, the mean value of each symbol in $\hat{\bf{f}}$ will be constant, however the FM demodulator output SNR will change, implying that output noise power $\sigma_s^2$ is modulated as a function of $|H|$.
...
\end{split}
\end{equation}
where $CNR$ is the mean (set point) channel carrier to noise ratio, and $|H|$ is the current magnitude of the fading channel.  We note that fading may push the input CNR beneath the demodulator threshold.  At this point the SNR drops rapidly, and the noise becomes impulsive rather than AWGN. Except for rare deep fades, our system is designed to operate above threshold.  For simplicity of training, we therefore approximate the demodulator SNR using a piecewise linear model that increases AWGN rapidly as CNR drops:
...
SNR_{dB} = \begin{cases}
	       ...
	       \end{cases}
\end{equation}
where $CNR^{\prime}_{dB} = CNR_{dB} + H_{dB}$. In this model, $SNR_{dB}=CNR_{dB}=9 \si{dB}$, i.e. over a 3dB range the FM ``processing gain" is reduced to zero and the demodulator starts to perform worse than a SSB product detector.

To calculate the noise power to apply in simulation we solve for $N=\sigma_s^2$:
\begin{equation}
\label{eq:fm_sigma}
\begin{split}
...
\begin{enumerate}
\item Select an average $CNR$ set point and vehicle speed.
\item Generate a sequence of $|H|$ samples based on a vehicle speed for this sequence.
\item Calculate $SNR$ for each point in the sequence using [\ref{eq:fm_snr}]. 
\item Estimate $\sigma_s$ for each point in the sequence using [\ref{eq:fm_sigma}].
\item Apply noise to each point in the sequence using [\ref{eq:fm_noise}].
\end{enumerate}
